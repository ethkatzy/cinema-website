...
def test_booking_without_csrf_token_is_rejected(client, make_user, db_path):
...
def test_booking_creates_booking_for_logged_in_user(client, db_path, make_user):
...
def test_booking_rejects_seat_already_booked_by_someone_else(client, db_path, make_user):
    seat_id = get_seat_ids(db_path, SHOWING_ID, limit=1)[0]
    first_user = make_user(email="first-booker@example.com")
    create_booking_directly(db_path, first_user["userid"], SHOWING_ID, seat_id)
...
def test_booking_rejects_seat_not_belonging_to_showings_screen(client, db_path, make_user):
...
def test_delete_booking_blocks_non_owner(client, db_path, make_user):
    owner = make_user(email="owner@example.com")
    attacker = make_user(email="attacker@example.com")
    seat_id = get_seat_ids(db_path, SHOWING_ID, limit=1)[0]
    bookingid = create_booking_directly(db_path, owner["userid"], SHOWING_ID, seat_id)
...
def test_delete_booking_requires_csrf(client, db_path, make_user):
...
def test_delete_booking_succeeds_for_owner(client, db_path, make_user):
...
def test_edit_page_blocks_non_owner(client, db_path, make_user):
...
def test_edit_confirm_blocks_non_owner(client, db_path, make_user):
...
def test_edit_confirm_updates_seats_and_info_for_owner(client, db_path, make_user):
    owner = make_user(email="editor@example.com")
    seat_ids = get_seat_ids(db_path, SHOWING_ID, limit=2)
    bookingid = create_booking_directly(db_path, owner["userid"], SHOWING_ID, seat_ids[0])
...
def test_edit_confirm_rejects_seat_booked_by_another_booking(client, db_path, make_user):
    owner = make_user(email="editor2@example.com")
    other_user = make_user(email="otheruser@example.com")
    seat_ids = get_seat_ids(db_path, SHOWING_ID, limit=2)
    my_bookingid = create_booking_directly(db_path, owner["userid"], SHOWING_ID, seat_ids[0])
    create_booking_directly(db_path, other_user["userid"], SHOWING_ID, seat_ids[1])
...
def test_edit_confirm_allows_keeping_own_already_booked_seat(client, db_path, make_user):
